...
const ClientsManagement = () => {
  const [clients, setClients] = useState([]); // current page items
  const [filteredClients, setFilteredClients] = useState([]); // legacy - mirrors clients
  const [totalCount, setTotalCount] = useState(0); // total matching clients on server
  ...
  // Load clients from API on mount/use dependencies
  useEffect(() => {
    loadClients();
  }, []);
  ...
  const loadClients = async () => {
    ...
    setError(null);
    try {
      // Use debounced search to avoid making requests on every keystroke
      ...
      // Server returns { items: [...], total: N }
      const clientsData = resp.items || [];
  ...
  // Reload clients when debounced search, filters, activity or page change
  useEffect(() => {
    // Reset to page 1 when filters/search change
    setCurrentPage(1);
  }, [debouncedSearch, filterPlan, filterStatus, filterActivity]);

  useEffect(() => {
    loadClients();
  }, [debouncedSearch, filterPlan, filterStatus, filterActivity, currentPage]);

  // Mirror clients into filteredClients for legacy UI logic
  useEffect(() => {
  ...
  const currentClients = clients; // current page items
  ...
  const handleCreateClick = () => {
    // Log to help debug click behavior in browser console
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      await Promise.all(
        selectedClients.map((id) => apiService.deleteClient(id))
      );
      await loadClients(); // Reload clients
      setSelectedClients([]);
    } catch (err) {
  ...
  const handleModalSave = async (clientData) => {
    try {
      ...
      if (editingClient) {
        ...
        savedClient = await apiService.updateClient(
          editingClient.id,
          updateData
        );
      } else {
        ...
        console.log("Creating client on server with:", newClientData);
        savedClient = await apiService.createClient(newClientData);
        console.log("Create client response:", savedClient);
        // Optimistic UI update: if the server returned the created client, prepend it to current page
        try {
          if (savedClient && savedClient.id) {
            const transformed = {
              id: savedClient.id,
              companyName: savedClient.company_name,
              contactPerson: savedClient.contact_person,
              email: savedClient.email,
              planType: savedClient.plan_type,
              status: savedClient.status === "active" ? "Active" : "Inactive",
              dateJoined: savedClient.date_joined
                ? new Date(savedClient.date_joined).toLocaleDateString()
                : "",
              lastActivity: savedClient.last_activity
                ? new Date(savedClient.last_activity).toLocaleDateString()
                : "Never",
              brandTone: savedClient.brand_tone,
              audienceType: savedClient.audience_type,
              marketingSuggestions: savedClient.marketing_suggestions,
            };

            setClients((prev) => [transformed, ...prev]);
            setFilteredClients((prev) => [transformed, ...prev]);
            setTotalCount((prev) => prev + 1);
            // Refresh in background to ensure server state consistency
            loadClients().catch((e) =>
              console.warn("Background reload failed:", e)
            );
          }
        } catch (optimErr) {
          console.warn("Optimistic update failed:", optimErr);
        }
      }
      // If optimistic update performed, still attempt a reload; if not, reload now
      if (!editingClient) {
        // We already triggered a background load above after create; ensure modal closes
        setShowModal(false);
        setEditingClient(null);
      } else {
      ...
      return savedClient; // Return saved client for document upload
    } catch (err) {
      setError(
        editingClient
      ...
      throw err; // Re-throw so ClientModal can handle it
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                ...
                <strong>{clientToDelete?.companyName}</strong>? This action
                cannot be undone.
              </p>
